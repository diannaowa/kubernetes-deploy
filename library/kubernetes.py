...
def api_request(module, url, method="GET", headers=None, data=None):
    ...
    # if method !="GET":
    #     module.fail_json(msg="res:%s,info:%s" %(response,info))
...
#add by zwliu@thoughtworks.com
def k8s_creage_update_resource(module,url,data):
    name = data.get('metadata', {}).get('name')
    if name is None:
        module.fail_json(msg="Missing a named resource in object metadata when trying to update a resource")
    info, body = api_request(module, url + "/" + name)
    if info['status'] >= 400:
        #module.fail_json(msg="failed to update the resource '%s': %s" % (name, info['msg']), url=url)
        #module.fail_json(msg="%s" % info)
        item_changed, item_body = k8s_create_resource(module, url, data)
    else:
        item_changed, item_body = k8s_update_resource(module, url, data)

    return item_changed, item_body

def k8s_delete_rc(module,url,data):
    name = data.get('metadata', {}).get('name')
    if name is None:
        module.fail_json(msg="Missing a named resource in object metadata when trying to update a resource")
    data.get('spec',{})['replicas'] = 0

    item_changed, item_body = k8s_update_resource(module, url, data)

    if item_changed:
        item_changed, item_body = k8s_delete_resource(module, url, data)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            state=dict(default="present", choices=["present", "absent", "update", "replace","create_update","delete_rc"])
        ),
    ...
    for item in data:
        ...
        elif state == 'create_update':
            item_changed, item_body = k8s_creage_update_resource(module, url, item)
        elif state == 'delete_rc':
            item_changed, item_body = k8s_delete_rc(module, url, item)
